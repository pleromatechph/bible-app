...
// LAST READ MEMORY RESTORATION ENGINE
...
// READER MEMORY RESTORATION ENGINE: Translation State
...
// Global Audio Speed State & Memory Restoration
...
// Track Note States for Unsaved Edits Detection
let initialPreviewNoteText = "";
let initialActionNoteText = "";

// STATIC SVG BLUEPRINTS (PREVENTS LUCIDE ICON RE-PARSING BLINKING/FLICKERING)
...
const closeFontSizeSheetBtn = document.getElementById('closeFontSizeSheetBtn');
...
// Smart Verse Range Formatter Engine
...
// Universal Copy Fallback Engine
  ...
  try {
    ...
    while (stmt.step()) {
      ...
      if (String(currentChap) === String(chapterNum)) {
        ...
        // DIRECT HEART ICON TAP LISTENER (Opens Preview Replica Sheet)
...
const themeList = ['theme-light', 'theme-dark', 'theme-sage', 'theme-blush', 'theme-sky'];
...
function applyAppTheme(themeName) {
  ...
  localStorage.setItem('sog_active_theme', themeName);

  const themeIcon = document.getElementById('themeIcon');
  if (themeIcon) {
    let iconName = 'sun';
    if (themeName === 'theme-dark') iconName = 'moon';
    else if (themeName === 'theme-sage') iconName = 'sprout';
    else if (themeName === 'theme-blush') iconName = 'heart';
    else if (themeName === 'theme-sky') iconName = 'cloud';
...
if (themeToggleBtn) {
  themeToggleBtn.addEventListener('click', () => {
    ...
    applyAppTheme(themeList[nextIndex]);
  });
}
...
// Backdrop Overlay Click Engine (Tap Outside to Auto-Close Modals)
if (drawerOverlay) {
...
if (minimizeReaderBtn) {
  minimizeReaderBtn.addEventListener('click', () => {
    ...
    // Step A: Trigger smooth downward slide-down transition
    if (readerView) readerView.classList.remove('open');
    setActiveNav(navDashboardBtn);
    if (dashboardView) dashboardView.classList.add('active');
    
    // Step B: Wait 350ms for transition animation before resetting dashboard state
    setTimeout(() => {
      closeAllModals();
      resetDashboardTabToStarted();
      
      // Restore floating mini player on dashboard if audio track is active
      if (audioElement && audioElement.currentTime > 0 && currentVersion === 'NIV + Audio') {
        if (miniAudioPlayer) miniAudioPlayer.style.display = 'flex';
      }
    }, 350);
...
function setActiveNav(btn) {
  [navDashboardBtn, navReaderBtn, navMoreBtn].forEach(b => {
    if (b) b.classList.remove('active');
  });
  if (btn) btn.classList.add('active');
}

// Sheets Modals Close Handler
...
// ACCURATE DIRTY-STATE "X" BUTTON HANDLER FOR NOTE EDIT MODE
...
[closeSettingsSheetBtn, closeVersionSheetBtn, closeFontSizeSheetBtn].forEach(el => {
...
function closeAllModals() {
  ...
  [
    settingsSheetModal, profileSheetModal, offlineLibraryModal, 
    versionSheetModal, fontSizeSheetModal, selectionModal, 
    feedbackSheetModal, favsModal, donateModal, 
    verseActModal, notePreviewSheetModal, drawerOverlay
  ].forEach(m => {
    if (m) m.classList.remove('open');
  });
}

/* MOBILE SWIPE LEFT GESTURE ENGINE (AUTO CLOSE / BACK) */
...
document.addEventListener('touchend', (e) => {
  ...
  if (deltaX < -60 && Math.abs(deltaY) < 40) {
    const activeOpenModal = [
      notePreviewSheetModal, verseActionSheetModal, profileSheetModal,
      offlineLibraryModal, favoritesSheetModal, donateSheetModal,
      feedbackSheetModal, versionSheetModal, fontSizeSheetModal,
      selectionModal, settingsSheetModal
    ].find(m => m && m.classList.contains('open'));

    if (activeOpenModal) {
      if (activeOpenModal === notePreviewSheetModal) {
        closeNotePreviewSheetBtn.click();
      } else if (activeOpenModal === settingsSheetModal) {
        closeAllModals();
        resetDashboardTabToStarted();
      } else {
        closeSubSheetToSettings(activeOpenModal);
      }
    }
  }
}, { passive: true });
...
// INSTANT ZERO-FLICKER OPEN ENGINE FOR DOWNLOAD MANAGER
...
const audioBookSizes = {
  ...
  "Ephesians": "11.8 MB", "Philippians": "4", "Colossians": "7.8 MB", "1 Thessalonians": "9.8 MB",
...
// COMPACT STARTED & FINISHED BOOK CARDS
    ...
    card.innerHTML = `
      <div class="progress-details">
          <div class="progress-title-row">
            ...
            <span class="progress-percent-badge">${pct}%</span>
          </div>
          <span class="progress-subtitle">${doneCount} of ${b.chapters} Chapters Completed</span>
...
const spiritualTaglines = [
  ...
  "Planted by Purpose, Flourishing in Christ",
  "Cultivating Faith, Nurturing Souls",
  "Deep Roots, Radiant Growth",
  "Sown in Love, Strong in the Lord",
  "From Small Seeds to Mighty Faith",
  "Nurturing Hearts to Blossom in Christ",
  "Anchored in Truth, Rising in Grace",
  "Grace Sown, Faith Grown",
  "Watered by the Word, Growing in the Light"
];
...
// Firebase Auth Listener (Awtomatikong tumatakbo kapag nag-sign in/out)
if (typeof firebase !== 'undefined' && firebase.auth) {
  firebase.auth().onAuthStateChanged((user) => {
    if (user) {
      ...
      // Auto-extract First Name kung walang naka-save na custom nickname
...
// Push Local Progress & Notes to Cloud (Firestore)
...
// Realtime Listener for Multi-Device Syncing
...
function renderUserGreetingBanner() {
  ...
  if (userGreetingTitle) {
    if (savedName) {
      ...
      const randomFallback = spiritualFallbackNames[Math.floor(Math.random() * spiritualFallbackNames.length)];
      userGreetingTitle.textContent = `Welcome back, ${randomFallback}! 👋`;
    }
  }
...
// Universal Audio Play / Pause Sync Engine
...
if (audioElement) {
  ...
  audioElement.addEventListener('ended', () => updatePlayIcons(false));
...
if (speedBtn) {
  speedBtn.addEventListener('click', (e) => {
    ...
    localStorage.setItem('sog_audio_speed_index', String(currentSpeedIndex)); // SAVE MEMORY
...
// Selectors
if (bookTabBtn) bookTabBtn.addEventListener('click', () => openBookSelector());
if (selectBookChapterBtn) selectBookChapterBtn.addEventListener('click', () => openBookSelector());
...
// IN-READER AUDIO DECK EXPLICIT TOGGLE ENGINE
if (readerMiniPlayer) {
...
// Dashboard Floating Mini Player Handlers
if (miniAudioPlayer) {
...
if (fontSizeSlider) {
  fontSizeSlider.addEventListener('input', () => {
    ...
    localStorage.setItem('sog_font_size', val); // SAVE MEMORY
  });
}
...
async function renderVersionOptionsList() {
  ...
  const installedCatalog = availableTranslations.filter(item => installedVersions.includes(item.code));
  ...
  installedCatalog.forEach(item => {
    ...
    div.addEventListener('click', () => {
      ...
      localStorage.setItem('sog_active_version', currentVersion); // SAVE MEMORY
...
// PURE FAVORITE CHECKER (INDEPENDENT FROM NOTES)
function isVerseFavorite(book, ch, verse) {
  ...
  const isFavParent = Object.keys(favs).some(key => {
    const item = favs[key];
...
// CHECK IF VERSE HAS PERSONAL REFLECTION NOTE
...
// READER VERSE TAP & DIRECT INTERACTION ENGINE
function attachVerseInteractions(element, book, ch, verse) {
  ...
  const handleTouchEnd = (e) => {
    ...
    if (!isLongPress) {
      ...
      // STRICT TAP OFFSET: Kapag pinitik ang Verse Number / Icon Area, WALA NANG AUTO-COPY NA MAGANAP
      ...
      if (selectedElements.length > 0) {
        ...
        const formattedCopy = `${book} ${ch}:${formattedVerseRangeStr} (${currentVersion})\n${compiledText}`;
...
// OPEN NOTE PREVIEW SHEET (CLEAN READ-VIEW ONLY WITHOUT HEADER HEART)
...
function openVerseActionSheet(book, ch, verse, rawElementText) {
  ...
  if (selectedElements.length > 1 && isTargetSelected) {
    ...
    activeSelectedVerse = { book, chapter: ch, verse: formattedVerseRangeStr, text: compiledText, isMultiple: true, selectedList: selectedElements };

    const titleEl = document.getElementById('verseActionTitle');
    if (titleEl) titleEl.textContent = `${book} ${ch}:${formattedVerseRangeStr}`;
  } else {
    activeSelectedVerse = { book, chapter: ch, verse, text: rawElementText.replace(/^\d+\s*/, ''), isMultiple: false };
    const titleEl = document.getElementById('verseActionTitle');
...
// ACCURATE HEART COLOR TOGGLE & DYNAMIC SVG RE-RENDER FIX
function updateFavoriteButtonUI() {
  ...
  // Pure check: Tinitignan lamang kung nasa favorite verses storage
  const favs = getFavoriteVerses();
...
if (toggleFavoriteBtn) {
  toggleFavoriteBtn.addEventListener('click', () => {
    ...
    // I-refresh ang Lucide icon pagkatapos ng state change
    setTimeout(() => {
      refreshIcons();
      updateFavoriteButtonUI();
    }, 50);
...
// SMART "X" BUTTON CLOSE HANDLER FOR LONG PRESS ACTION SHEET
function handleVerseActionSheetClose() {
  const noteInput = document.getElementById('verseNoteInput');
  const currentNoteVal = noteInput ? noteInput.value.trim() : '';

  // MAGAPATULOY SA WARNING KAPAG LAMANG MAY BINAGONG TEXT SA REFLECTION NOTE
...
// REAL-TIME SEARCH FILTER FOR FAVORITES & NOTES
...
/* UNIFIED FAVORITES & VERSE NOTES SYSTEM (CARD TAP VIEW + PURE TRASH ICON) */
function renderDashboardFavorites() {
  ...
  // 3. Render Cards (Card Tap = Preview)
  unifiedList.forEach(item => {
    ...
    // TRASH ICON CLICK HANDLER (WITH REALTIME READER SYNC RE-RENDER)
...
function restoreReaderMemoryState() {
  // 1. Restore Active Translation Label
